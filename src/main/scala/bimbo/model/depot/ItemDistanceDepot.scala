...
case class ItemDistanceDepot(covFuncParams: DenseVector[Double]) extends Metric[KnnPoint] {
  ...
  val logEllProductWeigth = exp(2 * covFuncParams(8))
   val logEllProductShortName = exp(2 * covFuncParams(9))

  def d(p1: KnnPoint, p2: KnnPoint): Double = {

    val d0 = p1.x(0) - p2.x(0)
      val d1 = if (p1.x(1) == p2.x(1)) 0.0 else 1
    ...
    val dShortProductName = if (p1.x(8) == p2.x(8)) 0.0 else 1
    (d0 * d0) / logEllLogSale + 
    d1 / logEllClientId + 
    d2 / logEllDepotId + 
    d3 / logEllChannelId + 
    ...
    dShortProductName / logEllProductShortName
